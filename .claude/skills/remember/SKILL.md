...
description: >
  ...
  defaults records to this agent's PERSONAL, PRIVATE scope (`--scope associate
...
scope:** the default personal scope is **private** (`--scope associate
...
- `--scope NAME` / `--visibility public|private` — record scope. **The wrapper
  ...
  (here, `associate`). Private records are served only to a recall in the same
